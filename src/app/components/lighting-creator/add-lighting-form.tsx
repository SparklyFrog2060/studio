...
import type { Lighting } from "@/app/lib/types";
...
const formSchema = z.object({
  ...
  type: z.enum(["bulb", "lamp", "led_strip", "other"]),
...
type LightingFormData = z.infer<typeof formSchema>;

interface AddLightingFormProps {
  onSubmit: (data: Omit<Lighting, "id" | "createdAt">) => void;
  isSaving: boolean;
  initialData?: Lighting;
}

const PREDEFINED_TAGS = ["Smart", "RGB", "Dimmable", "E27"];

export default function AddLightingForm({ onSubmit, isSaving, initialData }: AddLightingFormProps) {
  const { t } = useLocale();
  const [tagInput, setTagInput] = useState("");
  const [score, setScore] = useState(initialData?.score || 0);
  const isEditMode = !!initialData;

  const form = useForm<LightingFormData>({
  ...
  const handleFormSubmit = (data: LightingFormData) => {
    onSubmit({ ...data, score });
    if (!isEditMode) {
        form.reset();
    }
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{isEditMode ? t.editLighting : t.addLighting}</CardTitle>
      </CardHeader>
      <Form {...form}>
        <form onSubmit={form.handleSubmit(handleFormSubmit)}>
          <CardContent className="space-y-6">
                  ...
                  <FormControl>
                    ...
                    >
                      <FormItem className="flex items-center space-x-2 space-y-0">
                        <FormControl><RadioGroupItem value="bulb" /></FormControl>
                        <FormLabel className="font-normal">{t.bulb}</FormLabel>
                      </FormItem>
                      <FormItem className="flex items-center space-x-2 space-y-0">
                        <FormControl><RadioGroupItem value="lamp" /></FormControl>
                        <FormLabel className="font-normal">{t.lamp}</FormLabel>
                      </FormItem>
                       <FormItem className="flex items-center space-x-2 space-y-0">
                        <FormControl><RadioGroupItem value="led_strip" /></FormControl>
                        <FormLabel className="font-normal">{t.ledStrip}</FormLabel>
                      </FormItem>
                       <FormItem className="flex items-center space-x-2 space-y-0">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{t.lightingName}</FormLabel>
                    <FormControl>
                      <Input placeholder={t.lightingNamePlaceholder} {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
          ...
          <CardFooter className="justify-between">
            ...
            <Button type="submit" disabled={isSaving}>
              {isEditMode ? t.saveChanges : t.saveLighting}
            </Button>
          </CardFooter>
        </form>
      </Form>
