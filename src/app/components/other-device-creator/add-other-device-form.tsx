...
const formSchema = z.object({
  ...
  type: z.enum(["washing_machine", "dishwasher", "tv", "speaker", "robot_vacuum", "air_purifier", "other"]),
...
type OtherDeviceFormData = z.infer<typeof formSchema>;

interface AddOtherDeviceFormProps {
  onSubmit: (data: Omit<OtherDevice, "id" | "createdAt">) => void;
  isSaving: boolean;
  initialData?: OtherDevice;
}

const PREDEFINED_TAGS = ["Smart", "Wi-Fi", "Aplikacja mobilna"];

export default function AddOtherDeviceForm({ onSubmit, isSaving, initialData }: AddOtherDeviceFormProps) {
  const { t } = useLocale();
  const [tagInput, setTagInput] = useState("");
  const [score, setScore] = useState(initialData?.score || 0);
  const isEditMode = !!initialData;

  const form = useForm<OtherDeviceFormData>({
    ...
    defaultValues: initialData || {
      ...
      connectivity: "other_app",
      homeAssistantCompatibility: 3,
      tags: [],
      specs: [],
    },
  });
  ...
  const handleFormSubmit = (data: OtherDeviceFormData) => {
    onSubmit({ ...data, score });
    if (!isEditMode) {
        form.reset();
    }
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{isEditMode ? t.editOtherDevice : t.addOtherDevice}</CardTitle>
      </CardHeader>
      <Form {...form}>
        <form onSubmit={form.handleSubmit(handleFormSubmit)}>
          <CardContent className="space-y-6">
              ...
              render={({ field }) => (
                <FormItem className="space-y-3">
                  ...
                  <FormControl>
                    ...
                    >
                      <FormItem className="flex items-center space-x-2 space-y-0"><FormControl><RadioGroupItem value="washing_machine" /></FormControl><FormLabel className="font-normal">{t.washingMachine}</FormLabel></FormItem>
                      <FormItem className="flex items-center space-x-2 space-y-0"><FormControl><RadioGroupItem value="dishwasher" /></FormControl><FormLabel className="font-normal">{t.dishwasher}</FormLabel></FormItem>
                      <FormItem className="flex items-center space-x-2 space-y-0"><FormControl><RadioGroupItem value="tv" /></FormControl><FormLabel className="font-normal">{t.tv}</FormLabel></FormItem>
                      <FormItem className="flex items-center space-x-2 space-y-0"><FormControl><RadioGroupItem value="speaker" /></FormControl><FormLabel className="font-normal">{t.speaker}</FormLabel></FormItem>
                      <FormItem className="flex items-center space-x-2 space-y-0"><FormControl><RadioGroupItem value="robot_vacuum" /></FormControl><FormLabel className="font-normal">{t.robotVacuum}</FormLabel></FormItem>
                      <FormItem className="flex items-center space-x-2 space-y-0"><FormControl><RadioGroupItem value="air_purifier" /></FormControl><FormLabel className="font-normal">{t.airPurifier}</FormLabel></FormItem>
                      <FormItem className="flex items-center space-x-2 space-y-0"><FormControl><RadioGroupItem value="other" /></FormControl><FormLabel className="font-normal">{t.other}</FormLabel></FormItem>
                    </RadioGroup>
                  </FormControl>
                  <FormMessage />
                </FormItem>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{t.otherDeviceName}</FormLabel>
                    <FormControl>
                      <Input placeholder={t.otherDeviceNamePlaceholder} {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormControl>
                        ...
                        >
                          <FormItem className="flex items-center space-x-2 space-y-0"><FormControl><RadioGroupItem value="good" /></FormControl><FormLabel className="font-normal text-green-600">{t.good}</FormLabel></FormItem>
                          <FormItem className="flex items-center space-x-2 space-y-0"><FormControl><RadioGroupItem value="medium" /></FormControl><FormLabel className="font-normal text-yellow-600">{t.medium}</FormLabel></FormItem>
                          <FormItem className="flex items-center space-x-2 space-y-0"><FormControl><RadioGroupItem value="bad" /></FormControl><FormLabel className="font-normal text-red-600">{t.bad}</FormLabel></FormItem>
                        </RadioGroup>
                      </FormControl>
                    </FormItem>
                  )}
          ...
          <CardFooter className="justify-between">
            ...
            <Button type="submit" disabled={isSaving}>
              {isEditMode ? t.saveChanges : t.saveOtherDevice}
            </Button>
          </CardFooter>
        </form>
      </Form>
