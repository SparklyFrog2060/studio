...
import type { OtherDevice, Connectivity } from "@/app/lib/types";

interface OtherDeviceCardProps {
  device: OtherDevice;
  onDelete: (id: string) => void;
  onEdit: (device: OtherDevice) => void;
}

export default function OtherDeviceCard({ device, onDelete, onEdit }: OtherDeviceCardProps) {
  const { t } = useLocale();
  ...
  const getConnectivityInfo = (connectivity: Connectivity) => {
    switch (connectivity) {
      ...
      default: return { name: '', evaluation: 'medium' as const };
    }
  };
  ...
  const getTypeInfo = (type: OtherDevice['type']) => {
      switch (type) {
          case 'washing_machine': return t.washingMachine;
          case 'dishwasher': return t.dishwasher;
          case 'tv': return t.tv;
          case 'speaker': return t.speaker;
          case 'robot_vacuum': return t.robotVacuum;
          case 'air_purifier': return t.airPurifier;
      ...
      <CardHeader className="flex flex-row items-start justify-between">
        <div className="flex-1">
          <div className="flex items-baseline gap-2 flex-wrap">
           ...
           <p className="text-xs text-muted-foreground">{t.type}: {getTypeInfo(device.type)}</p>
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              ...
              <AlertDialogFooter>
                ...
                <AlertDialogAction onClick={() => onDelete(device.id)} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">{t.confirm}</AlertDialogAction>
              </AlertDialogFooter>
            </AlertDialogContent>
          </AlertDialog>
        </div>
      ...
      <CardContent className="flex-grow space-y-4">
        ...
        <div className="space-y-2">
          ...
          {device.connectivity && (
            <div className="flex justify-between items-center text-sm p-2 rounded-md bg-muted/40">
              <div className="flex items-center gap-2">
                {getEvaluationIcon(getConnectivityInfo(device.connectivity).evaluation)}
                <span>{t.connectivity}:</span>
                <span className="font-semibold">{getConnectivityInfo(device.connectivity).name}</span>
              </div>
            </div>
          )}
          <div className="flex justify-between items-center text-sm p-2 rounded-md bg-muted/40">
